...
# 适合在容器/无 GUI 环境中运行
# 使用 ./python.sh diningroom_add.py 运行
# 特性：随机化 diningroom 指定 prims，在多个平面上放置
# pair 差异通过 A/B 帧中的新增目标体现（A 为隐藏后，B 为隐藏前）
...
def run_diningroom_example(
    ...
    # 5. BasicWriter：输出到 diningroom add 目录
    ...
    for cam_path in camera_list:
        ...
        for frame_idx in range(total_frames):
            ...
            # 每个 pair（A/B 两帧）重新采样一次待隐藏 prim 集合
            ...
            if frame_idx % 2 == 0:
                ...
                if origin:
                    ...
                    # B 随机化除 chair 外的类别；chair 保持当前原位
                    prim_z_map = manual_randomize_objects(non_chair_prim_path_map, surface_bounds_list)
                    if use_z_compensation:
                        apply_z_compensation(prim_z_map, reverse_prim_map)
            else:
        ...
        for idx in range(pair_count):
            ...
            # 捕获顺序是 B(隐藏前) -> A(隐藏后)，导出时交换为 A/B 命名
            ...
            if coord_a_src_idx < actual_coords and coord_b_src_idx < actual_coords:
                ...
                for prim_path in selected_paths_for_pair:
                    # add 任务：A 为隐藏后（坐标 null），B 为隐藏前（记录真实坐标）
