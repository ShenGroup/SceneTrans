...
def run_kitchen_example(
    ...
    output_dir: str = "/workspace/output/bedroom1_shelf_move/3_items",
):
    """
    ...
    for cam_path in camera_list:
        ...
        for frame_idx in range(total_frames):
            ...
            # For each pair (A/B two frames), re-sample the prim set
            ...
            if not sampled_prim_map:
                print(f" ⚠️ Pair {pair_idx}: no valid sampled prims, skipping randomization on this frame")
                frame_coordinates.append({})
                frame_2d_coordinates.append({})
                writer.detach()
            ...
            # Step 1: randomize object position and rotation using USD API
            prim_z_map = manual_randomize_objects(sampled_prim_map, surface_bounds_list)

            # Step 2: optional Z compensation (when off, only forced Z is used)
            if use_z_compensation:
                apply_z_compensation(prim_z_map, reverse_prim_map)
            ...
            for path in sampled_paths:
                target_path = sampled_prim_map.get(path, path)
                prim = stage.GetPrimAtPath(target_path)
                if prim and prim.IsValid():
                    xformable = UsdGeom.Xformable(prim)
                    if xformable:
            ...
            current_coords = get_world_coordinates(sampled_paths, sampled_prim_map)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/bedroom1_shelf_move/3_items", help="Output directory")
