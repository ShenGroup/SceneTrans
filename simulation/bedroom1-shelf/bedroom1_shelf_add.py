...
def run_kitchen_example(
    ...
    output_dir: str = "/workspace/output/bedroom1_shelf_add/2_items",
):
    """
    Headless version of the kitchen scene example
    Only randomizes 3 specific bottle prims on the counter surface
    Each randomization randomly decides whether the bottle falls over (rotate 0 or 90 degrees around X axis)
    """
    ...
    # RTSubframes: force the renderer to re-sample on each frame
    rep.settings.carb_settings("/omni/replicator/RTSubframes", 64)
    print(" RTSubframes set to 16 (to eliminate ghosting)")



    # 4. Pair with on_frame trigger to let orchestrator drive write to disk
    ...
    def find_valid_bottle_prims():
        """Return all valid prims from the user-specified list"""
        specific_prims = [
    ...
    # 7. Check and remove object collision bodies (randomized objects don't need CollisionAPI)
    def remove_collision_from_bottles(bottle_paths):
        """Check and remove the object's CollisionAPI; randomized objects don't need collision bodies"""
        for path in bottle_paths:
            ...
            # Check and remove CollisionAPI from Xform itself
    ...
    def add_semantic_labels(prim_paths):
        """
        Add semantic labels to the specified prims, uniformly using ornament
        ...
        for path in prim_paths:
            ...
            # All target prims share the same semantic label
            prim_name = "ornament"
            ...
            try:
                ...
                # Add semantic label using rep.modify.semantics
    ...
    def normalize_prim_path(path: str) -> str:
        """Ensure the path starts with /, compatible with inputs without leading slash"""
    ...
    def get_surface_z_height(surface_prim_paths):
        """
        Get the highest Z coordinate of the surface as a reference height for object placement
        """
    ...
    # Get surface height (take the max from surface_z in bounds_list)
    # Note: get_surface_z_height may return 0, so we update again after computing bounds_list
    surface_z_height = get_surface_z_height(valid_cabinet_prims)

    # 10.2 Z compensation function: called after randomization + rotation, prevents the object from sinking into the plane
    def apply_z_compensation(prim_surface_z_map, prim_display_map=None):
        """
        Based on the object's current pose bounding box, compensate the Z height so the bottom sits flush on the surface
        ...
        for path, target_surface_z in prim_surface_z_map.items():
            ...
            # Compute the required Z adjustment: align bbox bottom to the surface_z of the plane this object is on
    ...
    # Whether to enable Z compensation (can be turned off if only forcing Z)
    ...
    def find_actual_mesh(prim_path, depth=0, max_depth=3):
        ...
        Because scatter_2d requires a direct Mesh prim, not a container holding Meshes
        """
        if depth > max_depth:
            return None
    ...
    for cabinet_prim in valid_cabinet_prims:
        ...
        if actual_mesh:
            ...
            # If no child Mesh found, try using the prim directly
    ...
    # If multiple surfaces, create a group; otherwise use the single surface directly
    ...
    def get_each_surface_bounds(surface_prim_paths):
        ...
        Use world transform matrix + Mesh extent attribute to compute the range precisely
        ...
        def find_mesh_in_prim(parent_prim, depth=0, max_depth=5):
            """Recursively find the Mesh child node under prim"""
            if depth > max_depth:
                return None
        ...
        for path in surface_prim_paths:
            ...
            # First print the world position of the prim (for diagnosis)
            xformable = UsdGeom.Xformable(prim)
            if xformable:
                world_transform = xformable.ComputeLocalToWorldTransform(0)
                translation = world_transform.ExtractTranslation()
            ...
            if local_extent and len(local_extent) >= 2:
                ...
                # Build the 8 corners of local space (for a 3D box) or 4 corners (for a plane)
                local_corners = [
    ...
    # 11.6 Pure USD API randomization function (full control, not relying on Replicator trigger)
    import random as py_random

    def manual_randomize_objects(prim_path_map, bounds_list):
        """
        Randomize object position and rotation using pure USD API
        Each object randomly selects a plane, then is placed within the bounds of that plane
        ...
        for orig_path, move_path in prim_path_map.items():
            ...
            # Record the surface Z height the object is placed on
            prim_surface_z_map[move_path] = surface_z

            # Randomly generate position (within the bounds of the selected plane)
            rand_x = py_random.uniform(min_x, max_x)
            rand_y = py_random.uniform(min_y, max_y)
            # Force Z directly, not relying on bbox compensation
            rand_z = surface_z
            ...
            # Verify position is within the bounds
    ...
    for cam_path in camera_list:
        ...
        # First run warmup frames to flush the temporal cache (remove residue from original positions)
        # warmup frames = 3 * warmup_k, ensuring it's a multiple of 3
        warmup_frames = 3 * warmup_k
        print(f" Running {warmup_frames} warmup frames (k={warmup_k}) with randomization...")
        rep.orchestrator.set_capture_on_play(False)

        # During warmup, directly move bottles to random positions using USD API
        ...
        for warmup_i in range(warmup_frames):
            # Move bottles once every 3 frames to refresh cache
            if warmup_i % 3 == 0:
                for path in valid_candidate_paths:
        ...
        # Diagnostic: pull one frame of segmentation data directly from annotator to confirm non-zero
        def log_annotator_sample(render_prod):
            ...
            # Manually step one frame while capture is off
            rep.orchestrator.step()
        ...
        def get_world_coordinates(prim_paths, prim_map=None):
            """Get world coordinates for the specified prims"""
            coords = {}
            for path in prim_paths:
                ...
                if prim and prim.IsValid():
                    ...
                    if xformable:
                        # Get world transform matrix
                        world_transform = xformable.ComputeLocalToWorldTransform(0)
                        # Extract translation (world coordinate)
                        translation = world_transform.ExtractTranslation()
        ...
        def get_2d_center_coordinates_from_projection(prim_paths, camera_path, image_width, image_height, prim_map=None):
            """
            Project 3D world coordinates to 2D pixel coordinates using the camera projection matrix (not relying on semantic segmentation)
            ...
            for path in prim_paths:
                ...
                # Camera looks at -Z, so z should be negative to be visible
                if cam_z >= 0:
                ...
                pixel_y = int((1 - ndc_y) * 0.5 * image_height)  # invert Y
        ...
        # 13-15. Use USD API to randomize directly, do not use Replicator trigger
        # This way we fully control when randomization runs, avoiding Z compensation being overwritten

        frame_counter = [0]
        frame_coordinates = []
        frame_2d_coordinates = []

        # Warmup step: run multiple frames to flush the renderer's temporal accumulation cache to eliminate ghosting
        # After each randomization run 3 frames to let the renderer stabilize, total 5 randomizations = 15 frames
        ...
        # 16. Main render loop
        # Before each capture, run a few frames to flush the temporal accumulation cache and eliminate ghosting between A/B frames
        flush_frames_before_capture = 6  # number of flush frames before capture (increased to 6 frames)
        ...
        for frame_idx in range(total_frames):
            ...
            # For each pair (A/B two frames), re-sample the prim set to be changed
            ...
            if frame_idx % 2 == 0:
                ...
                print(f" Pair {pair_idx}: selected {len(sampled_paths)} prim(s) for add pair: {sampled_paths}")

                # add task: Frame A should be "after hidden" state, Frame B should be "not hidden" state
                # First ensure all objects are visible and randomize positions, then hide sampled objects in Frame A
                ...
                # Randomize once per pair, A/B share the same pose
                prim_z_map = manual_randomize_objects(tracked_prim_map, surface_bounds_list)
                if use_z_compensation:
                    apply_z_compensation(prim_z_map, reverse_prim_map)

                # Frame A: hide the sampled objects (after hidden)
                ...
                tracked_paths = list(prim_path_map.keys())
                tracked_prim_map = prim_path_map
                # Frame B: do not move objects anymore, only restore the sampled objects to visible (not hidden)
                shown_paths = set_prims_visibility(sampled_paths, True, prim_path_map)
                print(f" Pair {pair_idx}: B frame shown {len(shown_paths)} prim(s): {shown_paths}")

            # Step 3: flush frames - detach writer, run several frames to flush temporal accumulation cache
            writer.detach()
            ...
            # Compute 2D pixel coordinates using camera projection
        ...
        for idx in range(pair_count):
            ...
            if coord_a_idx < actual_coords and coord_b_idx < actual_coords:
                ...
                # Build coordinates dict, containing world coordinates and 2D center pixel coordinates
                selected_paths_for_pair = pair_records[idx].get("selected_prims", [])
                for prim_path in selected_paths_for_pair:
                    # add task: Frame A is after hidden (null), Frame B is not hidden (has value)
...
def depth_npy_to_png(npy_path, png_path):
    """
    Convert a depth map npy file to a visualizable png image
    """
    import numpy as np
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/bedroom1_shelf_add/2_items", help="Output directory")
