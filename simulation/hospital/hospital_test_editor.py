...
def run_hospital_example_editor(
    ...
    def enable_physics_on_wheelchairs(wheelchair_paths):
        ...
        for path in wheelchair_paths:
            ...
            print(f"[hospital_test_editor] Physics ready: {path} (translate:{'Y' if translate_added else 'N'}, rotate:{'Y' if rotate_added else 'N'}, mesh colliders:{mesh_count})")
    ...
    for prim in stage.Traverse():
        ...
        for chair_name in specific_chairs:
            if chair_name in path_str:
                ...
                # 仅当是 Xform 时，记录为 scatter_2d 的 no_coll_prims 目标
                if prim_type == "Xform" and prim_name == chair_name:
                    chair_no_coll_xforms.add(path_str)
                break
    
    print(f"[hospital_test_editor] Chairs processed: CollisionAPI on {len(chair_collision_paths)} prim(s); avoiding {len(chair_no_coll_xforms)} Xform(s)")

    # 将需要避开的椅子 Xform 转成 replicator prim，供 scatter_2d 使用
    chair_no_coll_prims = []
    if chair_no_coll_xforms:
        for path in sorted(chair_no_coll_xforms):
            prim_node = rep.get.prims(path_pattern=path, prim_types=["Xform"])
            if prim_node:
                chair_no_coll_prims.append(prim_node)
        avoid_list = ", ".join(sorted(chair_no_coll_xforms))
        print(f"[hospital_test_editor] Will avoid collisions with chairs: {avoid_list}")
        ...
        def randomize_wheelchairs():
            ...
            with wheelchairs:
                scatter_kwargs = {
                    ...
                    "check_for_collisions": 1,
                }
                if chair_no_coll_prims:
                    scatter_kwargs["no_coll_prims"] = chair_no_coll_prims
                    print(f"  [Randomizer] Using no_coll_prims to avoid {len(chair_no_coll_prims)} chair(s)")
                else:
                    print("  [Randomizer] no_coll_prims empty; only avoiding wheelchair-to-wheelchair collisions")

                rep.randomizer.scatter_2d(**scatter_kwargs)
