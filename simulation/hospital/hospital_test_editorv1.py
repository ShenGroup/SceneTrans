...
def run_hospital_example_editor(
    ...
    # 如果你想用其他相机，可以修改这里
    desired_cameras = {"/OmniverseKit_Persp"}  # 只使用这一个相机
    camera_list = []
    ...
    def enable_physics_on_wheelchairs(wheelchair_paths):
        ...
        for path in wheelchair_paths:
            ...
            if not prim.HasAPI(UsdPhysics.CollisionAPI):
                ...
                print(f"[hospital_test_editor] Added CollisionAPI to: {path}")
    ...
    if not surface:
        ...
        print("[hospital_test_editor] Will use manual randomization instead")
        USE_SCATTER_2D = False
    else:
        USE_SCATTER_2D = True  # 默认使用 scatter_2d
    ...
    for cam_path in camera_list:
        ...
        print(f"[hospital_test_editor] Output directory: {cam_out_dir}")
        writer.initialize(output_dir=str(raw_dir), rgb=True, bounding_box_2d_tight=False)
        writer.attach(render_product)

        # 11. 配置物体随机化
        print(f"[hospital_test_editor] Using scatter_2d for {len(candidate_paths)} objects")
        print(f"[hospital_test_editor] Surface mesh: {actual_floor_mesh}")
        
        # 注意：这里必须使用找到的 actual_floor_mesh，不能用原始的 floor_prim
        # 因为 floor_prim 可能是 Xform 容器，scatter_2d 需要实际的 Mesh
        # surface 已经在外面创建好了，这里直接使用
        
        if not surface:
            print("[hospital_test_editor] Warning: No valid surface found for scatter_2d")
            continue
        ...
        print(f"[hospital_test_editor] Prepared {pair_count} pairs for {cam_path}; {len(candidate_paths)} objects.")

        # 10. 添加调试追踪机制
        randomization_tracker = {
            "call_count": 0,
            "frame_positions": {},  # 记录每帧每个物体的位置
        }
        
        # 选择随机化模式（已在前面根据 surface 是否有效决定）
        
        if USE_SCATTER_2D:
            # 方法 1: 使用 scatter_2d
            print(f"[hospital_test_editor] Using scatter_2d for randomization")
            
            def randomize_wheelchairs():
                """使用 scatter_2d 在地面上随机散布"""
                randomization_tracker["call_count"] += 1
                frame_num = randomization_tracker["call_count"]
                
                print(f"\n[RANDOMIZER] === Frame #{frame_num} ===")
                ...
                frame_positions = {}
                for path in candidate_paths:
                    prim_obj = stage.GetPrimAtPath(path)
                    if prim_obj:
                        ...
                        if xformable:
                            for op in xformable.GetOrderedXformOps():
                                if op.GetOpType() == UsdGeom.XformOp.TypeTranslate:
                                    pos = op.Get()
                                    frame_positions[path] = pos
                                    print(f"  {path}: ({pos[0]:.2f}, {pos[1]:.2f}, {pos[2]:.2f})")
                                    break
                
                randomization_tracker["frame_positions"][frame_num] = frame_positions
                
                return wheelchairs.node
        
        else:
            # 方法 2: 手动随机化（备选方案）
            print(f"[hospital_test_editor] Using manual randomization")
            
            # 获取地面的边界
            floor_prim_obj = stage.GetPrimAtPath(floor_prim)
            floor_mesh = UsdGeom.Mesh(floor_prim_obj)
            
            # 获取地面的实际边界
            points_attr = floor_mesh.GetPointsAttr()
            if points_attr:
                ...
                if points and len(points) > 0:
                    ...
                    y_floor = sum(ys) / len(ys)
                    
                    # 缩小范围，避免在边缘
                    padding = 20.0  # 从边缘向内缩进 20 单位
                    min_x += padding
                    max_x -= padding
                    min_z += padding
                    max_z -= padding
                    
                    print(f"[hospital_test_editor] Floor bounds (with padding):")
                    print(f"  X: [{min_x:.1f}, {max_x:.1f}]")
                    print(f"  Y: {y_floor:.1f} (floor height)")
                    ...
                    min_z, max_z = -100, 100
                    y_floor = 0
                    print(f"[hospital_test_editor] Warning: Could not get floor points, using default bounds")
            else:
                min_x, max_x = -100, 100
                min_z, max_z = -100, 100
                y_floor = 0
                print(f"[hospital_test_editor] Warning: Could not get floor points attribute, using default bounds")
            ...
            def randomize_wheelchairs():
                ...
                print(f"\n[RANDOMIZER] === Frame #{frame_num} (Manual Mode) ===")
                ...
                for path in candidate_paths:
                    ...
                    random_y = y_floor + 1.0  # 稍微抬高 1 单位避免穿模
                    ...
                    if prim_obj:
                        ...
                        if xformable:
                            ...
                            for op in xformable.GetOrderedXformOps():
                                if op.GetOpType() == UsdGeom.XformOp.TypeTranslate:
                                    op.Set((random_x, random_y, random_z))
                                    frame_positions[path] = (random_x, random_y, random_z)
                                    print(f"  {path}: pos=({random_x:.2f}, {random_y:.2f}, {random_z:.2f}), yaw={random_yaw:.1f}°")
                                    break
                            
                            # 设置旋转（只在 Z 轴上，垂直轴）
                            for op in xformable.GetOrderedXformOps():
                                if op.GetOpType() == UsdGeom.XformOp.TypeRotateXYZ:
                                    op.Set((0.0, 0.0, random_yaw))
                                    break
        ...
        print(f"\n[hospital_test_editor] ========================================")
        print(f"[hospital_test_editor] Timeline configured for {cam_path}:")
        ...
        print(f"[hospital_test_editor] Pair metadata saved to: {metadata_file}")
    ...
    print(f"=" * 80)
    print(f"\n使用方法:")
    print(f"1. 在 Timeline 中按 PLAY 按钮")
    print(f"2. 等待所有帧渲染完成（观察 Console 输出）")
    print(f"3. 渲染完成后，在 Script Editor 中运行: reorganize_pairs_from_raw()")
    print(f"\n期望的输出:")
    print(f"- 随机化器应该被调用 {total_frames} 次")
    print(f"- 每次调用都应该显示物体的新位置")
    print(f"- Console 中应该看到 [RANDOMIZER] === Frame #X === 的输出")
    print(f"\n" + "=" * 80)
    
    # 保存追踪器的引用（供后续检查）
    global _randomization_tracker
    _randomization_tracker = randomization_tracker
