...
def run_hospital_example_editor(
    ...
    for cam_path in camera_list:
        ...
        def randomize_wheelchairs():
            ...
            # 获取需要排除碰撞检测的物体（按语义类别）
            exclude_semantics = ['table', 'desk', 'chair', 'dispenser', 'shelf']
            no_coll_prims_list = []
            
            for semantic_class in exclude_semantics:
                try:
                    prims = rep.get.prims(
                    ...
                    if prims:
                        no_coll_prims_list.append(prims)
                except Exception:
                    pass  # 忽略找不到的语义类别
            
            # 应用随机化
            with wheelchairs:
                if no_coll_prims_list:
                    rep.randomizer.scatter_2d(
                        surface_prims=surface,
                        check_for_collisions=True,
                        # no_coll_prims=no_coll_prims_list
                    )
                else:
                    rep.randomizer.scatter_2d(
                        surface_prims=surface,
