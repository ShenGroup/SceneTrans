...
def run_kitchen_example(
    ...
    Only randomize 3 specific bottle prims, on the counter plane
    Each randomization randomly decides whether the bottle is fallen (rotate around the X axis 0 or 90 degrees)
    """
    ...
    def add_semantic_labels(prim_label_map):
        """
        Add semantic labels to the specified prims (pillow/lamp)
    ...
    # Semantic labels: pillow/lamp categorization
    ...
    def capture_initial_pose_map(prim_paths, prim_map=None):
        """Record initial translate/rotate values of prims, used to restore Frame A in each pair."""
        pose_map = {}
    ...
    def restore_pose_map(pose_map, prim_display_map=None):
        """Restore prim to the recorded initial pose."""
    ...
    for cam_path in camera_list:
        ...
        def write_pair_outputs_immediately(pair_idx, coords_a, coords_b, coords_2d_a, coords_2d_b):
            """
            Immediately organize and write to disk after each pair is generated, to avoid waiting for all pairs to finish.
            """
            import shutil
        ...
        # After warmup, restore to initial state to ensure Frame A is the initial scene
        ...
        for pair_idx in range(pair_count):
            ...
            # Frame B: only move the sampled pillows
            if sampled_prim_map:
