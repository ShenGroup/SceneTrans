...
def run_kitchen_example(
    ...
    output_dir: str = "/workspace/output/bedroom1_bed_remove/3_items",
):
    """
    Headless version of the bedroom scene example
    - origin=False: Frame A will randomly place controlled objects within the plane, Frame B hides the sampled objects
    - origin=True: Frame A keeps the initial state, Frame B only hides the sampled objects
    """
    ...
    for cam_path in camera_list:
        ...
        for frame_idx in range(total_frames):
            ...
            # Resample the prim set to remove at the start of each pair (A/B frames)
            ...
            if frame_idx % 2 == 0:
                ...
                # Frame A: first ensure all controlled objects are visible, then perform normal randomization
                set_prims_visibility(valid_candidate_paths, True, prim_path_map)
                ...
                if origin:
                    ...
                    # A only randomizes pillow; lamp keeps current original position
                    prim_z_map = manual_randomize_objects(pillow_prim_path_map, surface_bounds_list)
                    if use_z_compensation:
                        apply_z_compensation(prim_z_map, reverse_prim_map)
            else:
                ...
                tracked_paths = list(prim_path_map.keys())
                tracked_prim_map = prim_path_map
                # Frame B: no longer move objects, only hide the sampled objects
        ...
        # 17. Reorganize outputs (including RGB, depth maps, segmentation maps)
        import shutil
        ...
        # Verify that there are enough frames to create pairs
        ...
        for idx in range(pair_count):
            ...
            # Frame file index: starts from 0 directly
            frame_a_idx = idx * 2
            frame_b_idx = idx * 2 + 1

            # Fill in coordinate information (using coordinate index)
            ...
            if coord_a_idx < actual_coords and coord_b_idx < actual_coords:
                ...
                # Build coordinates dict, including world coordinates and 2D center pixel coordinates
                selected_paths_for_pair = pair_records[idx].get("selected_prims", [])
                for prim_path in selected_paths_for_pair:
                    # remove task: write null for hidden objects' coordinates in Frame B
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--origin",
        action="store_true",
        help="Keep A frame as original layout; only hide sampled prims in B frame",
    )
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/bedroom1_bed_remove/3_items", help="Output directory")
