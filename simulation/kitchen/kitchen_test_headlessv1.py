...
def run_kitchen_example(
    ...
    # # 禁用 DLSS（DLSS 使用时间历史，会产生鬼影）
    # rep.settings.carb_settings("/rtx/post/dlss/enabled", False)
    # print("[kitchen_headless] DLSS disabled")
    
    # # # 禁用 TAA（时间抗锯齿也会产生鬼影）
    # # rep.settings.carb_settings("/rtx/post/aa/op", 0)
    # # print("[kitchen_headless] TAA disabled")
    ...
    def add_semantic_labels(prim_paths):
        ...
        for path in prim_paths:
            ...
            try:
                ...
                added_count += 1
                print(f"[kitchen_headless] ✓ Semantic label added: {path} -> '{prim_name}'")
    ...
    for cam_path in camera_list:
        ...
        # 在这期间做几次随机化，但不捕获
        print(f"[kitchen_headless] Running {warmup_frames} warmup frames with randomization...")
        rep.orchestrator.set_capture_on_play(False)
        
        # 在 warmup 期间，直接用 rep.modify.pose 移动瓶子到随机位置
        # 这样可以刷掉原始位置的时间缓存
        ...
        # 16. 运行渲染
        print(f"[kitchen_headless] Running orchestrator...")
        ...
        if len(rgb_frames) >= total_frames:
            for idx, record in enumerate(pair_records):
                ...
                print(f"[kitchen_headless] Created {record['pair_id']}: rgb, depth, instance_seg, semantic_seg")
            
            # 清理剩余的 raw frames
            for leftover in raw_dir.glob("*.*"):
                leftover.unlink(missing_ok=True)
        else:
            print(f"[kitchen_headless] Expected {total_frames} frames, found {len(rgb_frames)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warmup-frames", type=int, default=10, help="Number of warmup frames")
