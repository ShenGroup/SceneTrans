...
def run_kitchen_example(
    ...
    def find_bottle_prims():
        specific_prims = [
            ...
            # "/root/Kitchen_Bottle_02",
            # "/root/Kitchen_Bottle_03",
            # "/root/Kitchen_Bottle_04",
    ...
    for cam_path in camera_list:
        ...
        def randomize_bottles():
            ...
            # 在 randomize_bottles() 开头加这个
            for path in candidate_paths:
                prim = stage.GetPrimAtPath(path)
                if prim and prim.IsValid() and path in initial_transforms:
                    xformable = UsdGeom.Xformable(prim)
                    if xformable:
                        xformable.SetLocalTransformation(initial_transforms[path])
