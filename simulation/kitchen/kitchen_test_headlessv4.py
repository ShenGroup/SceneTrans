...
def run_kitchen_example(
    ...
    for cam_path in camera_list:
        ...
        def get_2d_center_coordinates(bbox_data, target_prim_paths):
            """
            从 bounding_box_2d_tight annotator 数据中提取指定 prims 的2D中心像素坐标
            
            Args:
                bbox_data: bounding_box_2d_tight.get_data() 返回的数据
                target_prim_paths: 需要获取坐标的 prim 路径列表
            
            Returns:
                dict: {prim_path: [center_x, center_y]} 或 {prim_path: None} 如果未找到
            """
            coords_2d = {}
            ...
            if bbox_data is None:
                print("[kitchen_headless] Warning: bbox_data is None")
                return coords_2d
            
            # 获取数据和信息
            data = bbox_data.get("data", None)
            info = bbox_data.get("info", {})
            prim_paths = info.get("primPaths", [])
            
            if data is None or len(data) == 0:
                print("[kitchen_headless] Warning: No bounding box data available")
                return coords_2d
            ...
            for i, bbox in enumerate(data):
                ...
                # 检查是否是我们关心的 prim
                if prim_path in target_prim_paths:
                    # 从 bbox 中提取 x_min, y_min, x_max, y_max
                    ...
                    coords_2d[prim_path] = [center_x, center_y]
                    print(f"[kitchen_headless]   2D center for {prim_path}: ({center_x}, {center_y}) from bbox [{x_min},{y_min},{x_max},{y_max}]")
        ...
        for frame_idx in range(total_frames):
            ...
            current_2d_coords = get_2d_center_coordinates(bbox_data, candidate_paths)
        ...
        # 分离 bounding box annotator
        bbox_2d_annotator.detach()
