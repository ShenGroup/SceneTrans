...
# Suitable for running in container / non-GUI environments
# Run with ./python.sh kitchen_test_headless.py
# Features: randomize 3 bottle prims, place on two counter planes
...
def run_kitchen_example(
    ...
    Only randomizes 3 specific bottle prims on the counter planes
    Each randomization randomly decides whether the bottle falls (rotate 0 or 90 degrees around X axis)
    """
    ...
    # 3.2 Remove the physics scene and disable global physics, to avoid objects being pushed away by physics after randomization
    ...
    # 4. Combined with the on_frame trigger, let the orchestrator drive write to disk
    ...
    if cam_prim and cam_prim.IsValid():
        ...
        # Try to find other cameras in the scene as a fallback
    ...
    # 6. Only use the 3 specified bottle prims for randomization
    ...
    def remove_collision_from_bottles(bottle_paths):
        """Check and remove the object's CollisionAPI, randomized objects do not need a collision body"""
        for path in bottle_paths:
            ...
            # Recursively check all child nodes' CollisionAPI
            def remove_collision_from_children(parent_prim, depth=0):
                ...
                for child in parent_prim.GetChildren():
                    ...
                    # Continue recursive check of child nodes
    ...
    def add_semantic_labels(prim_paths):
        ...
        1. Directly write semantic attributes via USD primvars (take effect immediately)
        ...
        for path in prim_paths:
            ...
            # Automatically assign semantic labels based on the prim path (take the last segment)
    ...
    # Remove the physics collision bodies of these prims (if any)
    ...
    # Verify the cabinet prims exist
    valid_cabinet_prims = []
    ...
    def apply_z_compensation(prim_surface_z_map):
        """
        Based on the object's current pose bounding box, compensate the Z height so that the bottom touches the surface

        Core logic:
        1. Get the Z value in the object's current translate op
        2. Compute the gap between the bbox bottom and the target surface Z
        3. Adjust Z so that the bbox bottom = the surface_z of the plane the object is on

        Args:
            prim_surface_z_map: {prim_path: surface_z} the plane Z height at which each object is placed
        """
        if not prim_surface_z_map:
            print(f"\n[Z-comp] ⚠️ No objects to compensate")
            return
        ...
        for path, target_surface_z in prim_surface_z_map.items():
            prim = stage.GetPrimAtPath(path)
            if not prim or not prim.IsValid():
                print(f"[Z-comp] ❌ Invalid Prim: {path}")
                continue

            xformable = UsdGeom.Xformable(prim)
            if not xformable:
                print(f"[Z-comp] ❌ Unable to get Xformable: {path}")
                continue
            ...
            if not translate_op:
                print(f"[Z-comp] ⚠️ {path}: no translate op, skipping")
                continue

            current_pos = translate_op.Get()
            if not current_pos:
                print(f"[Z-comp] ⚠️ {path}: translate op has no value")
                continue
            ...
            if not world_bbox or world_bbox.GetRange().IsEmpty():
                print(f"[Z-comp] ⚠️ Unable to compute bbox: {path}")
                continue
    ...
    # Get cabinet prims as surfaces for scatter_2d
    surface_nodes = []
    ...
    def get_each_surface_bounds(surface_prim_paths):
        ...
        Use the world transform matrix + the Mesh's extent attribute to precisely compute the range
        Returns: [(min_x, max_x, min_y, max_y, surface_z), ...]
        """
        surface_bounds_list = []

        print(f"\n[Surface-Bounds] ========== Computing range of each surface ==========")
        ...
        for path in surface_prim_paths:
            ...
            if local_extent and len(local_extent) >= 2:
                ...
                # Build 8 corner points in local space (for a 3D box) or 4 corners (for a plane)
                local_corners = [
    ...
    def manual_randomize_objects(prim_paths, bounds_list):
        ...
        for path in prim_paths:
            prim = stage.GetPrimAtPath(path)
            if not prim or not prim.IsValid():
                print(f"[Manual-Rand] ❌ Invalid Prim: {path}")
                continue
            ...
            rand_z = surface_z  # Place at the surface height first, Z compensation will adjust later
            ...
            # Verify the position is within the range
    ...
    for cam_path in camera_list:
        ...
        # First run warmup frames to flush the temporal cache (remove the residual image at the original position)
        # warmup frame count = 3 * warmup_k, ensure a multiple of 3
        ...
        # During warmup, directly move bottles to random positions via USD API
        # This flushes the temporal cache at the original positions
        # Move once every 3 frames, total warmup_k moves
        import random as py_random
        for warmup_i in range(warmup_frames):
            # Move bottles once every 3 frames to refresh the cache
            if warmup_i % 3 == 0:
        ...
        # Diagnostic: directly pull one frame of segmentation data from the annotator, confirm non-zero
        ...
        for pair_idx in range(pair_count):
            pair_records.append({
                ...
                "camera": cam_path,
                "change_type": "add",
                "coordinates": {},  # A hidden coordinate may be None, B is the visible coordinate
            })

        print(f"[kitchen_headless] Generating {pair_count} pairs ({total_frames} frames)")

        # Define the function to get world coordinates
        ...
        def get_2d_center_coordinates_from_projection(prim_paths, camera_path, image_width, image_height):
            """
            Use the camera projection matrix to project 3D world coordinates to 2D pixel coordinates
            """
            coords_2d = {}
        ...
        # This way we have full control over when randomization is executed, avoiding Z compensation being overwritten
        ...
        for warmup_i in range(warmup_randomizations):
            ...
            # Run multiple frames after each randomization to let temporal accumulation refresh
        ...
        # 16. Main rendering loop (A/B pair: A is the visible state after random placement, B is the state after random hiding)
        ...
        for pair_idx in range(pair_count):
            ...
            # Ensure all prims are in the visible state
            for path in candidate_paths:
                prim = stage.GetPrimAtPath(path)
                if prim and prim.IsValid():
                    prim.GetAttribute("visibility").Set("inherited")

            # Record coordinates in the visible state (used as Frame B)
            ...
            # Randomly decide which prims to hide (Frame A is the hidden state)
            any_hidden = False
            hide_map = {}
            for path in candidate_paths:
                prim = stage.GetPrimAtPath(path)
            ...
            # Restore visibility, to facilitate capturing B
            for path in candidate_paths:
                prim = stage.GetPrimAtPath(path)
                if prim and prim.IsValid():
                    prim.GetAttribute("visibility").Set("inherited")

            # Capture Frame B (visibility restored)
            writer.detach()
            for _ in range(flush_frames_before_capture):
                rep.orchestrator.step()
            writer.attach(render_product)
            ...
            # Record metadata: A is the hidden state, B is the visible state
            ...
            for prim_path in candidate_paths:
                if any_hidden and hide_map.get(prim_path):
                    # Hidden object: A has no coordinates, B is the visible coordinate
