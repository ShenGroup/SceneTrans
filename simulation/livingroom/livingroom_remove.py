...
# Run with ./python.sh livingroom_remove.py
# Features: randomize specified livingroom prims, place on multiple surfaces
# Pair difference is expressed via the removal target in Frame A / Frame B
...
def run_livingroom_example(
    ...
    output_dir: str = "/workspace/output/livingroom_remove/3_items",
):
    """
    Headless version of the bedroom scene example
    - origin=False: Frame A randomly places controlled objects on surfaces, Frame B hides the sampled objects
    ...
    # 5. BasicWriter: output to the livingroom remove directory
    ...
    for cam_path in camera_list:
        ...
        for frame_idx in range(total_frames):
            ...
            # For each pair (Frame A / Frame B), resample the set of prims to remove
        ...
        for idx in range(pair_count):
            ...
            if coord_a_idx < actual_coords and coord_b_idx < actual_coords:
                ...
                for prim_path in selected_paths_for_pair:
                    # remove task: write null for the coordinates of objects hidden in Frame B
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/livingroom_remove/3_items", help="Output directory")
