# Headless version - adapted from the livingroom scene
# Suitable for running in container / no-GUI environments
# Run with ./python.sh livingroom_move.py
# Features: randomize specified livingroom prims, place on multiple surfaces
# Pair difference is expressed via the same batch of objects being at different random positions in Frame A / Frame B
...
def run_livingroom_example(
    ...
    output_dir: str = "/workspace/output/livingroom_move_origin/3_items",
):
    """
    Headless version of the livingroom scene example
    Generate move data on the specified livingroom prim set
    - origin=False: generate A (initial) / B (after move) frames following the normal flow
    - origin=True: force each pair's Frame A to be restored and kept as the initial state
    """
    ...
    # 5. BasicWriter: output to the livingroom move directory
    writer = rep.writers.get("BasicWriter")
    out_dir = Path(output_dir)

    # 5.1 Static objects to keep hidden throughout the entire process (hidden before generation starts and repeatedly enforced during the process)
    always_hidden_prims = [
        "/Root/Meshes/livingroom_767839/book_0030",
        "/Root/Meshes/livingroom_767839/book_0031",
        "/Root/Meshes/livingroom_767839/book_0032",
    ]

    def enforce_hidden_prims(prim_paths):
        """Force the given prim and its renderable child nodes to be invisible."""
        for prim_path in prim_paths:
            prim = stage.GetPrimAtPath(prim_path)
            if not prim or not prim.IsValid():
                print(f" Always-hidden prim not found: {prim_path}")
                continue
            ...
            print(f" ✓ Enforced hidden prim: {prim_path} (imageables={hidden_count})")

    # Hide before any randomization or rendering
    enforce_hidden_prims(always_hidden_prims)
    ...
    def add_semantic_labels(prim_label_map):
        """
        Add semantic labels to the specified prims, set by category as book/table/vase/plate/tv_box
    ...
    move_target_labels = set(valid_prim_groups.keys()) - {"table"}
    ...
    print(
        ...
        f"(move-targets(all_except_table): {len(selected_candidate_paths)}; {category_summary})"
    )
    ...
    def capture_initial_pose_map(prim_paths, prim_map=None):
        """Record the initial translate/rotate values of prims, used to restore each pair's Frame A."""
        pose_map = {}
    ...
    def restore_pose_map(pose_map, prim_display_map=None):
        """Restore prims to the recorded initial pose."""
    ...
    for cam_path in camera_list:
        ...
        # During warmup, directly move bottles to random positions using the USD API
        ...
        for warmup_i in range(warmup_frames):
            # Move bottles once every 3 frames to flush the cache
            if warmup_i % 3 == 0:
                for path in valid_candidate_paths:
        ...
        def write_pair_outputs_immediately(pair_idx, coords_a, coords_b, coords_2d_a, coords_2d_b):
            """
            Organize and write to disk immediately after each pair is generated, avoiding having to wait for all pairs to finish.
            """
            import shutil
            ...
            # Wait for the current pair's Frame A / Frame B to actually be written to disk
            ...
            if frame_a_idx < len(rgb_frames) and frame_b_idx < len(rgb_frames):
                ...
                print(
                    f" Pair {pair_idx}: RGB frames not ready "
        ...
        # After warmup ends, restore to the initial state, ensuring Frame A is the initial scene
        ...
        # 16. Main render loop
        # Before each capture, run a few frames to flush the temporal accumulation cache, eliminating ghosting between Frame A and Frame B
        ...
        for pair_idx in range(pair_count):
            ...
            # Frame A:
            # - origin=True: restore and keep the initial state
            # - origin=False: perform one random placement on the sampled objects
            if origin:
                restore_pose_map(initial_pose_map, reverse_prim_map)
                ...
                print(f" Pair {pair_idx}: no valid sampled prims, A keeps current state")
            enforce_hidden_prims(always_hidden_prims)
            writer.detach()
            ...
            # Frame B: perform another random placement on the sampled objects
            if sampled_prim_map:
                prim_z_map = manual_randomize_objects(sampled_prim_map, surface_bounds_list)
                if use_z_compensation:
                    apply_z_compensation(prim_z_map, reverse_prim_map)
            else:
                print(f" Pair {pair_idx}: no valid sampled prims, B keeps initial state")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/livingroom_move_origin/3_items", help="Output directory")
