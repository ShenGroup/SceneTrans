# Headless 版本 - 基于 livingroom 场景改编
# 适合在容器/无 GUI 环境中运行
# 使用 ./python.sh livingroom_move.py 运行
# 特性：随机化 livingroom 指定 prims，在多个平面上放置
# pair 差异通过同一批物体在 A/B 帧的不同随机位置体现
...
def run_livingroom_example(
    ...
    """
    Headless 版本的 livingroom 场景示例
    在指定 livingroom prim 集合上生成 move 数据
    - origin=False: 按常规流程生成 A(初始)/B(移动后) 帧
    - origin=True: 强制每个 pair 的 A 帧恢复并保持为初始状态
    """
    ...
    # 5. BasicWriter：输出到 livingroom move 目录
    writer = rep.writers.get("BasicWriter")
    out_dir = Path(output_dir)

    # 5.1 全流程保持隐藏的静态物体（在生成开始前隐藏，并在流程中反复强制）
    always_hidden_prims = [
    ...
    def enforce_hidden_prims(prim_paths):
        """强制将给定 prim 及其可渲染子节点设为不可见。"""
        for prim_path in prim_paths:
            prim = stage.GetPrimAtPath(prim_path)
            if not prim or not prim.IsValid():
                print(f" ⚠️ Always-hidden prim not found: {prim_path}")
                continue
    ...
    def add_semantic_labels(prim_label_map):
        """
        为指定的 prims 添加语义标签，按类别设置 book/table/vase/plate/tv_box
        同时使用两种方式确保兼容性：
    ...
    for cam_path in camera_list:
        ...
        for pair_idx in range(pair_count):
            ...
            # A 帧：
            # - origin=True: 恢复并保持初始状态
            # - origin=False: 对抽中的物体执行一次随机放置
            if origin:
                restore_pose_map(initial_pose_map, reverse_prim_map)
