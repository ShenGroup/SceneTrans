...
def run_kitchen_example(
    ...
    """
    Headless 版本的卧室场景示例
    - origin=False: 每个 pair 先生成 B 帧（隐藏前），再生成 A 帧（隐藏后）
    - origin=True: 保持原始布局，仅执行可见性切换来生成 B/A
    """
    ...
    def add_semantic_labels(prim_label_map):
        """
        为指定的 prims 添加语义标签，按类别设置 book/chair/computer/table/camera/lamp
        同时使用两种方式确保兼容性：
    ...
    for cam_path in camera_list:
        ...
        for frame_idx in range(total_frames):
            ...
            # 每个 pair（B/A 两帧）重新采样一次待隐藏 prim 集合
        ...
        for idx in range(pair_count):
            ...
            # 每个 pair 的捕获顺序为 B(隐藏前) -> A(隐藏后)
            # 坐标索引和帧文件索引相同（warmup 不产生文件）
            coord_b_idx = idx * 2
            coord_a_idx = idx * 2 + 1
            
            # 帧文件索引：直接从 0 开始，且与 B/A 顺序一致
            frame_b_idx = idx * 2
            frame_a_idx = idx * 2 + 1
