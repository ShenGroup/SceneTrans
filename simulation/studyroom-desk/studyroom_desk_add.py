...
def run_kitchen_example(
    ...
    output_dir: str = "/workspace/output/studyroom_desk_add/3_items",
):
    """
    Headless version of the bedroom scene example
    - origin=False: each pair first generates Frame B (before hiding), then Frame A (after hiding)
    - origin=True: keep the original layout, only perform visibility toggle to generate B/A
    """
    ...
    non_lamp_candidate_paths = []
    for label, paths in valid_prim_groups.items():
        if label != "lamp":
            non_lamp_candidate_paths.extend(paths)
    ...
    # lamp stays in place; the remaining categories participate in random placement on the plane
    non_lamp_prim_path_map = {path: prim_path_map[path] for path in non_lamp_candidate_paths if path in prim_path_map}
    print(f" Non-lamp prims for plane randomization: {len(non_lamp_prim_path_map)}")
    ...
    for cam_path in camera_list:
        ...
        for warmup_i in range(warmup_frames):
            ...
            if (not origin) and warmup_i % 3 == 0:
                for path in non_lamp_candidate_paths:
        ...
        for warmup_i in range(warmup_randomizations):
            if origin:
                ...
                prim_z_map = manual_randomize_objects(non_lamp_prim_path_map, surface_bounds_list)
        ...
        for frame_idx in range(total_frames):
            ...
            if frame_idx % 2 == 0:
                ...
                print(f" Pair {pair_idx}: selected {len(sampled_paths)} prim(s) to hide for A: {sampled_paths}")
        ...
        for idx in range(pair_count):
            ...
            # EN pair EN B(EN) -> A(EN)
            # EN（warmup EN）
            coord_b_idx = idx * 2
            coord_a_idx = idx * 2 + 1
            
            # EN:start from 0 start, EN B/A EN
            frame_b_idx = idx * 2
            frame_a_idx = idx * 2 + 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/studyroom_desk_add/3_items", help="Output directory")
