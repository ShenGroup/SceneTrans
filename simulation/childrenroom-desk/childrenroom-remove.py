...
def run_kitchen_example(
    ...
    output_dir: str = "/workspace/output/childrenroom_desk_remove/3_items/120-139",
):
    """
    Headless EN
    EN3EN prims, EN
    ...
    def apply_z_compensation(prim_surface_z_map, prim_display_map=None):
        ...
        for path, target_surface_z in prim_surface_z_map.items():
            ...
            xformable = UsdGeom.Xformable(prim)
            if not xformable:
                print(f"[Z-comp] ❌ EN Xformable: {display_name}")
                continue
            ...
            if not translate_op:
                print(f"[Z-comp] ⚠️ {display_name}: EN translate op, EN")
                continue
            
            current_pos = translate_op.Get()
            if not current_pos:
                print(f"[Z-comp] ⚠️ {display_name}: translate op EN")
                continue
            ...
            world_bbox = bbox_cache.ComputeWorldBound(prim)
            if not world_bbox or world_bbox.GetRange().IsEmpty():
                print(f"[Z-comp] ⚠️ EN bbox: {display_name}")
                continue
    ...
    for cam_path in camera_list:
        ...
        for pair_idx in range(pair_count):
            ...
            # step 1:EN（EN, EN A/B EN）
            prim_z_map = manual_randomize_objects(prim_path_map, surface_bounds_list)
            
            # step 2:EN Z compensation（EN Z）
            if use_z_compensation:
                apply_z_compensation(prim_z_map, reverse_prim_map)
            ...
            writer.attach(render_product)
            rep.orchestrator.step()
            print(f"[kitchen_headless] Pair {pair_idx}: captured A (visible)")
            ...
            writer.attach(render_product)
            rep.orchestrator.step()
            print(f"[kitchen_headless] Pair {pair_idx}: captured B (hidden)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="/workspace/output/childrenroom_desk_remove/3_items/120-139", help="Output directory")
