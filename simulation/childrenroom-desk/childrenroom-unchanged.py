...
# unchanged 版本：A/B 使用同一帧（无任何变化），用于构建 AB 没有改变的数据集
...
def run_kitchen_example(
    ...
    out_dir = Path("/workspace/output/childrenroom_desk_remove/3_items/120-139")
    ...
    for cam_path in camera_list:
        ...
        # 创建 pair 计划（unchanged：每个 pair 只捕获 1 帧，该帧同时作为 A 与 B）
        ...
        print(f"[kitchen_headless] Generating {pair_count} pairs ({total_frames} frames, A==B)")
        ...
        # 16. 主渲染循环（unchanged 版本）
        # 每个 pair：随机化一次并捕获 1 帧；该帧同时作为 A 与 B（不做 hide、不做额外 randomize）
        ...
        for pair_idx in range(pair_count):
            ...
            # 步骤 1：随机化物体位置和旋转（仅一次，用于 A/B 同一位置）
            prim_z_map = manual_randomize_objects(prim_path_map, surface_bounds_list)
            
            # 步骤 2：可选 Z 补偿（关闭时仅使用强制 Z）
            if use_z_compensation:
                apply_z_compensation(prim_z_map, reverse_prim_map)
            ...
            writer.attach(render_product)
            rep.orchestrator.step()
            print(f"[kitchen_headless] Pair {pair_idx}: captured frame (A==B)")

            # 验证捕获后物体的实际位置
            print(f"[Verify] ===== A 帧位置验证 =====")
            for path in candidate_paths:
                target_path = prim_path_map.get(path, path)
                prim = stage.GetPrimAtPath(target_path)
                if prim and prim.IsValid():
        ...
        # unchanged：每个 pair 只捕获 1 帧
        min_frames_needed = pair_count
        ...
        if actual_rgb < min_frames_needed:
            ...
            # 尽可能创建能创建的 pair 数量
            pair_count = min(pair_count, actual_rgb)
        ...
        for idx in range(pair_count):
            ...
            # unchanged：A/B 使用同一帧、同一坐标
            coord_idx = idx
            frame_idx = idx
            
            # 填充坐标信息（使用坐标索引）
            actual_2d_coords = len(frame_2d_coordinates)
            if coord_idx < actual_coords:
                coords_a = frame_coordinates[coord_idx]
                coords_b = frame_coordinates[coord_idx]
                
                # 获取2D坐标（如果可用）
                coords_2d_a = frame_2d_coordinates[coord_idx] if coord_idx < actual_2d_coords else {}
                coords_2d_b = frame_2d_coordinates[coord_idx] if coord_idx < actual_2d_coords else {}
            ...
            # RGB（unchanged：同一帧同时作为 A/B）
            if frame_idx < len(rgb_frames):
                shutil.copy2(rgb_frames[frame_idx], pair_dir / "A_rgb.png")
                shutil.copy2(rgb_frames[frame_idx], pair_dir / "B_rgb.png")
            
            # 深度图
            if frame_idx < len(depth_frames):
                depth_ext = depth_frames[frame_idx].suffix
                shutil.copy2(depth_frames[frame_idx], pair_dir / f"A_depth{depth_ext}")
                shutil.copy2(depth_frames[frame_idx], pair_dir / f"B_depth{depth_ext}")
                
                # 如果是 npy 格式，生成 png 可视化
                if depth_ext == ".npy":
                    depth_npy_to_png(depth_frames[frame_idx], pair_dir / "A_depth.png")
                    depth_npy_to_png(depth_frames[frame_idx], pair_dir / "B_depth.png")
            
            # 实例分割图
            if frame_idx < len(instance_frames):
                shutil.copy2(instance_frames[frame_idx], pair_dir / "A_instance_segmentation.png")
                shutil.copy2(instance_frames[frame_idx], pair_dir / "B_instance_segmentation.png")
            
            # 语义分割图
            if frame_idx < len(semantic_frames):
                shutil.copy2(semantic_frames[frame_idx], pair_dir / "A_semantic_segmentation.png")
                shutil.copy2(semantic_frames[frame_idx], pair_dir / "B_semantic_segmentation.png")
            
            # 保存 metadata
            meta_path = pair_dir / "metadata.json"
            meta_path.write_text(json.dumps(record, indent=2))
            
            print(f"[kitchen_headless] Created {record['pair_id']}: A=frame{frame_idx}, B=frame{frame_idx} (A==B)")
